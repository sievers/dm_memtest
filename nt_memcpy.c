...
// Assumes that the len in bytes is divisible by 128
...
//this is now for copies from main memory into cache
void t_memcpy(void* dest, void* src, size_t len)
{
  // Assumes dest is 16 byte alligned
  // NOTE: This must be true or else non-temporal writes are not possible.
  assert( (((uintptr_t)dest) & 0xF) == 0 );
  assert( (len % 128) == 0 );
    ...
    if ((((uintptr_t)src) & 0xF) == 0) {
        t_aligned_memcpy(dest, src, len);
    } else {
        t_unaligned_memcpy(dest, src, len);
    }
}

// Assumes that the source is 16 byte alligned.
void t_aligned_memcpy(void* dest, void* src, size_t len)
{
    size_t num_loops = len >> 7;
    ...
    for (int i = 0; i < num_loops; ++i) {
      ...
      //_mm_prefetch( ((const char *) src) + 832, _MM_HINT_NTA );
        ...
        __m128i xmm_reg6 = _mm_load_si128(src_p + 6);
        __m128i xmm_reg7 = _mm_load_si128(src_p + 7);
        _mm_store_si128(dest_p, xmm_reg0);
        _mm_store_si128(dest_p + 1, xmm_reg1);
        _mm_store_si128(dest_p + 2, xmm_reg2);
        _mm_store_si128(dest_p + 3, xmm_reg3);
        _mm_store_si128(dest_p + 4, xmm_reg4);
        _mm_store_si128(dest_p + 5, xmm_reg5);
        _mm_store_si128(dest_p + 6, xmm_reg6);
        _mm_store_si128(dest_p + 7, xmm_reg7);
        src_p += 8;
        dest_p += 8;
    }
}
...
// Assumes that the source is not 16 byte alligned.
void t_unaligned_memcpy(void* dest, void* src, size_t len)
{
    size_t num_loops = len >> 7;
